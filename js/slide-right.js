$(function(){
	...
	function start(){
		$("#list li").eq(0).siblings().css({"right":"0px"});//slide 이미지의 첫번째요소와 형제들에게 css위치값을 저장한다.
		setInterval(function(){slide();},2000);//2초 간격으로 slide함수를 호출한다.
	};
	
	function slide(){
		now=now==imgs?0:now+=1//변수 now와 변수img의 값이 같다면 now의 값에 1을 더한 값을 대입한다.
		$("#list li").eq(now-1).css({"right":"3500px"});//이전이미지의 위치는 밖으로 나가게 위치값을 준다.
		$("#list li").eq(now).css({"right":"0px"});//현재이미지의 위치는 보일 수 있게 위치값을 준다.
	};
});